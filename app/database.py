...
def _migrate_social_study_groups_table(conn, backend: str) -> None:
    if backend == "mysql":
        cur = conn.cursor()
        try:
            cur.execute(
                """
                CREATE TABLE IF NOT EXISTS social_study_groups (
                  id VARCHAR(80) PRIMARY KEY,
                  universite VARCHAR(80) NOT NULL,
            ...
            pass
        for idx_sql in (
            "CREATE INDEX idx_study_groups_campus ON social_study_groups(universite, created_at)",
        ):
            try:
                cur.execute(idx_sql)
                conn.commit()
    ...
    try:
        conn.executescript(
            ...
            CREATE INDEX IF NOT EXISTS idx_study_groups_campus ON social_study_groups(universite, created_at DESC);
            """
        )
        conn.commit()
    except sqlite3.OperationalError:
...
def _migrate_monitor_metrics_snapshots_table(conn, backend: str) -> None:
    if backend == "mysql":
        cur = conn.cursor()
        try:
            cur.execute(
                """
                CREATE TABLE IF NOT EXISTS monitor_metrics_snapshots (
                  id VARCHAR(80) PRIMARY KEY,
                  cpu DOUBLE NULL,
                  ram DOUBLE NULL,
                  rpm DOUBLE NULL,
        ...
        try:
            cur.execute(
                "CREATE INDEX idx_monitor_metrics_created ON monitor_metrics_snapshots(created_at)"
            )
            conn.commit()
        except pymysql.err.OperationalError as exc:
            if exc.args[0] not in (1061, 1060):
    ...
    try:
        conn.executescript(
            ...
            CREATE INDEX IF NOT EXISTS idx_monitor_metrics_created ON monitor_metrics_snapshots(created_at);
            """
        )
        conn.commit()
    except sqlite3.OperationalError:
